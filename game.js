...
let classes = ['confetti-container', 'overlay', 'win-modal', 'lose-modal', 'keyboard', 'play-again', 'play-again-lose', 'help-button', 'help-modal', 'close-modal'], elems = {}, states = ['correct', 'present', 'absent'];
...
let grid, words, wordNum, numOfCorrectLetters, chosen, entered, letterStates = {};
...
const createGrid = function () {
  ...
  grid.style.gridTemplateColumns = `repeat(${chosen.length}, 75px)`;
  grid.style.maxWidth = `${chosen.length * 85}px`;
}
...
const openModal = function () {
  elems.helpModal.classList.remove('hidden');
  elems.overlay.classList.remove('hidden');
}

const closeModal = function () {
  elems.helpModal.classList.add('hidden');
...
const createEventListeners = function () {
  ...
  elems.helpButton.addEventListener('click', openModal);
  elems.overlay.addEventListener('click', closeModal);
  elems.closeModal.addEventListener('click', closeModal);
